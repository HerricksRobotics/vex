...
#pragma config(Sensor, dgtl3,  bumper,         sensorDigitalIn)
#pragma config(Sensor, dgtl12, jumper,         sensorDigitalIn)
#pragma config(Motor,  port1,           leftBackMotor, tmotorVex393_HBridge, openLoop)
...
void autoSkills()
{
	move('B', 4, true);  // moves back to the fence until bumper is activated, 4 sec is max time
	move('F', .2, false);
	wait1Msec(600);
	putUpLift();
	putDownLift();
	move('F', 3, false); // position so that shovel is in the starting box
	// reload star(s) and repeat
	move('B', 4, true);
	move('F', .2, false);
	wait1Msec(600);
	putUpLift();
