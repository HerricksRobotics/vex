...
#pragma config(Motor,  port9,           grabber,   tmotorVex393_MC29, openLoop)										//may want 2 motors to control the forklift
...
void openGrabber()
{
	motor[grabber] = 0;         				//CHANGE 0 TO A VALUE
	wait1Msec(250);
	motor[grabber] = 0;
}
...
//to detect if an object is in the grabber motor
//0 if not pressed
//1 if pressed
long toGrab()
{
	...
	while (true)
	{
  	...
  	if (vexRT[Ch3] > -35 && vexRT[Ch3] < 35)						//Deadlock Zones
		...
		if (vexRT[Btn5D] == 1) {							//moving lift down
			motor[liftLeftTop] = -127;					//one of these lifts has to be reversed.... change values if neccessary
			motor[liftLeftBottom] = -127;
			motor[liftRightTop] = 127;
			motor[liftRightBottom] = 127;
		}
		...
		if (vexRT[Btn6D] == 1)
		{
			motor[grabber] = -70;					//may want to make this faster...
		}
